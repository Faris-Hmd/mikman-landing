  ...
  return (
    <section id="plans" className="py-20 sm:py-24 relative overflow-hidden bg-bg/90 transition-colors duration-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Plan Cards Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 sm:gap-6 max-w-7xl mx-auto items-stretch">
          {tiers.map((tier, idx) => (
            <motion.div
              key={idx}
            ...
            >
              ...
              {idx === 0 ? (
                /* Free plan — link to app, no WhatsApp */
                <a
                  href="https://app.mikman.net"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center py-3.5 px-6 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-sm border border-border bg-surface text-text-primary hover:bg-surface-card hover:border-primary/40"
                >
                  {tier.cta}
                </a>
              ) : (
                /* Paid plans — WhatsApp link */
                <a
                  href={`https://wa.me/${t("whatsapp")}?text=${encodeURIComponent(
                    t("whatsappMsg").replace("%s", tier.name)
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className={`flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-300 shadow-sm ${
                    tier.highlight
                      ? "bg-emerald-600 hover:bg-emerald-500 text-white"
                      : "border border-emerald-500/30 bg-emerald-500/10 text-emerald-600 hover:text-emerald-500 hover:bg-emerald-500/15 hover:border-emerald-500/40"
                  }`}
                >
